...
PredictionMapping = dict[str, list[tuple[PolaritySentence, str]]]
...
def gather_classifier_predictions(predicting_model: NeuralClassifier, prediction_dataset: DataLoader,
    ...
    for batch, _, _, batch_kwargs in tqdm(prediction_dataset, file=stdout, disable=tqdm_bool):
    ...
    write_predictions(prediction_filepath, sentences, predictions, prediction_kwargs["prediction_format"])


def write_predictions(prediction_filepath: Optional[Union[str, TextIO]], sentences: list[PolaritySentence],
                      predictions: list[str], prediction_format: str):
    sorted_predictions: PredictionMapping = sort_predictions(sentences, predictions)
            ...
            for source, predicted_sentences in sorted_predictions.items():
                *_, source_filename = source.split("/")
                with open(f"{prediction_filepath}/{source_filename}", mode="w+", encoding="utf-8") as output_tsv_file:
                    output_string: str = ""
                    for (sentence, prediction) in predicted_sentences:
...
def sort_predictions(sentences: list[PolaritySentence], predictions: list[str]) -> PredictionMapping:
    predictions_by_file: PredictionMapping = {}
    for i in range(0, len(sentences)):
        sentence: PolaritySentence = sentences[i]
        prediction: str = predictions[i]
        predicted_sentence: tuple[PolaritySentence, str] = (sentence, prediction)
